...
\item \textbf{The per-block induction is circular} (\S\ref{sec:circular}):
  ...
  proven --- the formalization exposes a single residual crux
  (\lean{BlockBoundaryExists}) --- and the natural devices for discharging
  that crux are \emph{refuted} by an explicit astronomical one-edge ladder
  built in closed form (\lean{ladder\_exists}; Theorem~\ref{thm:ladder}).
...
\end{gather*}
the integers that follow $W_{202}$ backward integrally for $m$ rounds
(each round consumes the word's $q = 22$ digits of $3$-adic precision; the
$+2$ is the mod-$9$ base).  Pleasingly, at $m=1$ the canonical residue is
exactly $a^{*} = 1 + 3^{22}$ (\lean{aS202\_decomp}), of bit-size $35$.
...
The graph-level start is $\InvStart(m) = (0,\ a^{*} \bmod 3^{R})$
(\lean{InvStart}, \lean{aS202}); a vertex is a \emph{goal} if its cylinder
contains $1$, i.e.\ $c \equiv 1 \pmod{3^{R+j}}$ (\lean{InvVertex.IsGoal}).
...
separation oracle walks the graph).  Every infeasibility
...
$m$-cycles for $m \le 91$ \cite{Hercher2023}, atop the computational
verification frontier of \cite{Barina2021}; our instances demonstrate the
verified pipeline at deliberately small frontiers.)
...
Call an admissible-word step a \emph{carry step} if
$\size(3c+1) = \size(c) + 2$ (the excess case of the lossy size law).  Is
there an effective $\delta > 0$ and $\ell_0$ such that every admissible word
of length $\ell \ge \ell_0$ contains at least $\delta\,\ell$ carry steps?
Equivalently: an effective positive lower density of carries in the base-$2$
Sturmian coding of multiplication by $3$ along admissible orbits.  Any such
bound strengthens the size laws coefficient-wise, pushing the closed-form
barrier past the bit-criterion wall ($Q \approx 7.7m$ on the faithful tower,
\S\ref{sec:positive}) and breaking Theorem~\ref{thm:wall} from outside the
linear class.
\end{question}
...
\item[\textbf{T1$'$}] Lean theorems whose proofs invoke the compiled
  ...
  (\lean{Cert\_m1\_*} and seven early modules, inventoried in
  ...
  at most the three standard axioms; twelve former \lean{native\_decide}
...
\item \textbf{The split crux}: a non-canonical construction of the
  predicate \lean{BlockBoundaryExists} that survives
  Theorem~\ref{thm:ladder}, or a proof that no choice function over paths
  can supply it.
...
\lean{no\_hybrid\_potential\_m1\_Q9} & \lean{HybridPotentialNoGo}\\
